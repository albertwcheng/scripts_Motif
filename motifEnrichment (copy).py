...
def hashSequence1MM(element,content,N,CGBinKey):
...
def keySequenceByCGValue(DictNmer,filename,headerRow1,startRow1,fromCol1,toCol1):
...
def hashSequenceIntoBin(DictNmer,N,CGBinSize):
		...
		for element in perHeader.keys():
			...
			while iseq<nSeqGroups:
				...
				while iseq<nSeqGroups and filled<CGBinSize:
					...
					for content in sequencesToAdd:
						hashSequence1MM(perElement,content,N,CGBinKey)
						CGBin=CGBins[CGBinKey];
						CGBin["CGValues"].append(CGVal);				
...
def buildModel1MM(DictNmer,filename,headerRow1,startRow1,fromCol1,toCol1,N,CGBinSize):
	#DictNmer[header][element]["nseq"]=number of sequences
	...
	keySequenceByCGValue(DictNmer,filename,headerRow1,startRow1,fromCol1,toCol1);
	hashSequenceIntoBin(DictNmer,N,CGBinSize);
...
def findEnrichedMotifBy1MM(filename,headerRow1,startRow1,fromCol1,toCol1,N,CGBinSize,FDRCutOff,pvalueCutOff,topCount,NmerList):


	DictNmer=dict(); 

	buildModel1MM(DictNmer,filename,headerRow1,startRow1,fromCol1,toCol1,N,CGBinSize);
...
def printUsageAndExit(programName):
	...
	print >> stderr,"1st Markov Model\t1m filename fromCol toCol N-mer CGBinSize"
...
if __name__=="__main__":
	...
	if(program=="1m"):
		...
		filename, fromCol1, toCol1, Nmer, CGBinSize=args[1:]
		...
		findEnrichedMotifBy1MM(filename,headerRow,startRow,min(fromCol1)+1,max(toCol1)+1,Nmer,CGBinSize,FDRCutOff,pvalueCutOff,topCount,NmerList);
	#elif(program=="h"):
	#	print >> sys.stderr, "args[]:",args;
